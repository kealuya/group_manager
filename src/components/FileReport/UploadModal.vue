  ...
  <div>
    ...
    >
      ...
      <div style="display: flex;align-items: center;justify-content: start">
        <div>文档名称：</div>
        <el-input :disabled="mode==UPLOAD_MODAL_MODE.UPLOAD" v-model="item.docName" size="small"
                  style=";width: 450px" />
      </div>
...
import { callNewDoc, callUpdateDoc } from "@/api/fileReport/doc";
...
// ==============================================================================
// 组件 v-modal 模式
const props = defineProps<{ modelValue: boolean, mode: string, item: DocFile }>();
...
const httpRequest = (options: UploadRequestOptions): any => {
  ...
  // 按照规则拼接filename
  let fileName = userStore.userInfo.code + "-" + Date.now() + "." + props.item.docType;
  ...
  if (props.mode === UPLOAD_MODAL_MODE.NEW) {
    ...
    obsClient.putObject({
      ...
      if (result.CommonMsg.Status < 300) {
        ...
        let myFile: DocFile = props.item;
        myFile.fileName = fileName;
        myFile.updateUser = userStore.userInfo.code;
